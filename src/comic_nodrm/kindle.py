...
class Kindle:
    ...
    def decrypt_epub(self, book: Path, folder: Path, output_name: str | None = None) -> bool:
        """
        ...
        Args:
            ...
            output_name (str | None, optional): output name without suffix. Defaults to None.
        ...
        with TemporaryDirectory(dir=folder) as tmpdir:
            finished = decryptBook(str(book), tmpdir, [str(self.KINDLE_KEY)], [], [], [])
    ...
    def books(self) -> list[Path]:
        return [book for book in self.kindledir.glob("**/*") if book.suffix in self.KINDLE_EXT]

    @staticmethod
    def get_title(azwpath: str | Path) -> str | None:
        try:
